...
class ScdDataset(Dataset):
    def __init__(self, root_dir, phase="train", image_size=256, labeled_num=3):
...
def getPath(root, phase, labeled_num):
    ...
    for index in range(2):
        img_dir = os.path.join(root + Vendor + classes[index], '%s_img'%phase)
        tmp_img = walk(dirname=img_dir)
        if phase == "train":
            img_dir = img_dir + '/slices'
        vendor_img = walk(dirname=img_dir)

        label_dir = os.path.join(root + Vendor + classes[index], '%s_label'%phase)
        if phase == "train":
            label_dir = label_dir + '/slices'
        vendor_label = walk(dirname=label_dir)

        vendors = np.array([0]).repeat(len(vendor_img))
...
# 确定标签id
def get_lbid(root="../SCD/", phase="train", labeled_num=4, seq_paths=None, paths=None):
    # 构造采样列表
    ed_size = len(paths[0][0])
    lb_volid = list(range(0, labeled_num))
    cl = ['ED', 'ES']
    target_id = []
    for vi in lb_volid:
...
def get_scd_loaders(root_dir=r'F:/datasets/ACDC/', labeled_num=7, labeled_bs=8, batch_size=24, batch_size_val=16,
                     num_workers=4, worker_init_fn=None, train_transforms=None, val_transforms=None, num_classes=4):

    db_train = ScdDataset(root_dir=root_dir, phase="train", image_size=513, labeled_num=labeled_num)
    db_val = ScdDataset(root_dir=root_dir, phase="val", image_size=513, labeled_num=labeled_num)
