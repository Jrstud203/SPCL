...
class RandomRotation(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        if isinstance(img, Tensor):
            if isinstance(fill, (int, float)):
                fill_i = [float(fill)] * F.get_image_num_channels(img)  # F._get_image_num_channels(img)
            else:
                fill_i = [float(f) for f in fill]
        if isinstance(label, Tensor):
            if isinstance(fill, (int, float)):
                fill_l = [float(fill)] * F.get_image_num_channels(label)  # F._get_image_num_channels(img)
            else:
                fill_l = [float(f) for f in fill]
        angle = self.get_params(self.degrees)
        if torch.rand(1).item() < self.p:
            return F.rotate(img, angle, self.resample, self.expand, self.center, fill_i), F.rotate(label, angle, self.resample, self.expand, self.center, fill_l)
        return img, label
