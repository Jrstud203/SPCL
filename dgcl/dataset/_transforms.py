...
class RandomGenerator_sae(object):
    """对图片，标签和先验进行同一的转换，包括：翻转，旋转，缩放到224"""
    def __init__(self, output_size, p_flip=0.5, p_rot=0.5):
        self.output_size = output_size
        self.p_flip = p_flip
        self.p_rot = p_rot
    ...
    def __call__(self, sample):
        image, label, prior = sample['image'], sample['label'], sample['prior'] # 160,160 | 160,160 | 4,160,160
        if torch.rand(1) < self.p_flip:
            image, label, prior = self.random_rot_flip(image, label, prior)
        elif torch.rand(1) < self.p_rot:
            image, label, prior = self.random_rotate(image, label, prior)
        x, y = image.shape
        # 当order=0时，使用最近插值重新采样2倍
        image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        label = zoom(label, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        prior = zoom(prior, (1, self.output_size[0] / x, self.output_size[1] / y), order=0)
        image = torch.from_numpy(image.astype(np.float32)).unsqueeze(0)
        label = torch.from_numpy(label.astype(np.uint8)).unsqueeze(0)
        prior = torch.from_numpy(prior.astype(np.float32))
        sample['image'] = image
        sample['label'] = label
        sample['prior'] = prior
        return sample
    ...
    @staticmethod
    def random_rot_flip(image, label, prior):
        k = np.random.randint(0, 4)
        image = np.rot90(image, k) # axes=(0, 1)
        label = np.rot90(label, k)
        prior = np.rot90(prior, k, (1, 2))
        axis = np.random.randint(0, 2)
        image = np.flip(image, axis=axis).copy()
        label = np.flip(label, axis=axis).copy()
        prior = np.flip(prior, axis=axis+1).copy()
        return image, label, prior

    @staticmethod
    def random_rotate(image, label, prior):
        angle = np.random.randint(-20, 20)
        image = ndimage.rotate(image, angle, order=0, reshape=False) #  axes=(1, 0)
        label = ndimage.rotate(label, angle, order=0, reshape=False)
        prior = ndimage.rotate(prior, angle, axes=(2, 1), order=0, reshape=False)
        return image, label, prior
...
class RandomFlip_sae(torch.nn.Module):
    ...
    def forward(self, sample):
        if torch.rand(1) < self.p:
            img, label, prior = sample['image'], sample['label'], sample['prior']
            if self.direction == 'horizontal':
                sample['image'] = F.hflip(img)
                sample['label'] = F.hflip(label)
                sample['prior'] = F.hflip(prior)
            elif self.direction == 'vertical':
                sample['image'] = F.vflip(img)
                sample['label'] = F.vflip(label)
                sample['prior'] = F.vflip(prior)
            else:
                if torch.rand(1) < 0.5:
                    sample['image'] = F.hflip(img)
                    sample['label'] = F.hflip(label)
