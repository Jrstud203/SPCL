...
def RandomRescale(img,label,prior,minScale,maxScale):
    ...
    if len(img.size())==4:
        ...
        scalePriorOther=transforms.Resize((targetH_prior,targetW_prior), interpolation=InterpolationMode.BILINEAR)(prior[:,1:chs_prior,:,:])
        scalePriorFirst=transforms.Resize((targetH_prior,targetW_prior), interpolation=InterpolationMode.BILINEAR)(prior[:,0:1,:,:])
        ...
        for index in range(z):
            ...
            scaleImg=transforms.Resize((targetH,targetW), interpolation=InterpolationMode.BILINEAR)(img[:,index,:,:])#缩放较大时改用二维线性插值
            scaleLabel=transforms.Resize((targetH,targetW),interpolation=InterpolationMode.NEAREST)(label[:,index,:,:])
        ...
        chs,h,w=img.size()
        targetH,targetW=int(h*scaleRange),int(w*scaleRange)
        scaleImg=transforms.Resize((targetH,targetW),interpolation=InterpolationMode.NEAREST)(img)
        scaleLabel=transforms.Resize((targetH,targetW),interpolation=InterpolationMode.NEAREST)(label)
...
def centerCrop128Slice(img,label):
    if (label[:,:]==1).sum()>0: 
        ...
        indexWL, indexHL = label.shape[0] // 2, label.shape[1] // 2
    #cropRange=64#裁剪的范围
    #HShift=18
    cropRange=80#裁剪的范围
    HShift=24
